...
function showNotification(message, type = 'info') {
  ...
  // Remover após 5 segundos (exceto para training que é importante)
  if (type !== 'training') {
    setTimeout(() => {
...
async function trainModel(symbol) {
  try {
    showLoading(true)
    showNotification(
      `🧠 Treinando modelo LSTM para ${symbol}... Isso pode levar 2-5 minutos. Por favor aguarde!`,
      'training'
    )
    ...
    if (data.status === 'success') {
      showNotification(
        `✅ Modelo ${symbol} treinado com sucesso em ${duration}s! MAPE: ${data.metrics?.MAPE?.toFixed(
          2
        )}%`,
        'success'
      )
      ...
      showNotification(
        `ℹ️ Modelo ${symbol} já existe e está pronto para uso`,
        'info'
      )
    } else {
      showNotification(
        `❌ Erro no treinamento: ${data.message || 'Training failed'}`,
        'error'
      )
    }
    ...
    const errorMsg =
      error.name === 'AbortError'
        ? 'Timeout - treinamento ainda pode estar em progresso'
        : 'Erro de conexão'
...
function updateStatusCards(data) {
  ...
  if (data && data.model_info && data.model_info.training_data) {
    document.getElementById('trainingData').textContent = Math.round(
      data.model_info.training_data
    ).toLocaleString()
  }
}
...
function updatePriceChart(historicalData, predictions) {
  ...
  if (!historicalData || !historicalData.data) {
    container.innerHTML =
      '<p class="text-slate-400 text-center py-8">No historical data available</p>'
    return
  }
...
function updatePredictionsTable(predictions) {
  ...
  if (!predictions || !predictions.predicted_prices) {
    container.innerHTML =
      '<p class="text-slate-400 text-center py-8">No predictions available</p>'
    return
  }
  ...
  const tableHtml = `
          ...
          ${predictions.predicted_prices
            .map(pred => {
              ...
              <tr class="hover:bg-slate-700/30 transition-colors">
                <td class="px-4 py-3 text-sm text-white">${new Date(
                  pred.date
                ).toLocaleDateString()}</td>
                <td class="px-4 py-3 text-sm text-white font-medium">${formatCurrency(
    ...
    <div class="mt-4 text-xs text-slate-400">
      <p><strong>Model Confidence:</strong> <span class="${getConfidenceColor(
        predictions.confidence
      )}">${getConfidenceLabel(predictions.confidence)}</span></p>
      <p><strong>Prediction Time:</strong> ${new Date(
...
function updateModelMetrics(predictions) {
  ...
  if (!predictions || !predictions.model_metrics) {
    container.innerHTML =
      '<p class="text-slate-400 text-center py-8">No metrics available</p>'
    return
  }
  ...
  const metricsHtml = `
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div class="bg-slate-700/30 rounded-lg p-4 text-center">
        <div class="text-2xl font-bold ${getConfidenceColor(
          predictions.confidence
        )}">${metrics.MAPE?.toFixed(2) || 'N/A'}%</div>
      ...
      <div class="grid grid-cols-2 gap-4 text-sm">
        <div>
          ...
          <span class="text-white ml-2">${
            predictions.model_info?.model_type || 'LSTM'
          }</span>
        </div>
        <div>
          <span class="text-slate-400">Sequence Length:</span>
          <span class="text-white ml-2">${
            predictions.model_info?.sequence_length || 'N/A'
          }</span>
        </div>
        <div>
          <span class="text-slate-400">Training Data:</span>
          <span class="text-white ml-2">${
            predictions.model_info?.training_data || 'N/A'
          } points</span>
        </div>
        <div>
          ...
          <span class="${getConfidenceColor(
            predictions.confidence
          )} ml-2">${getConfidenceLabel(predictions.confidence)}</span>
        </div>
      </div>
    </div>
  `
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  document
    ...
    .addEventListener('click', makePredictions)
  document
    .getElementById('trainBtn')
    .addEventListener('click', trainModelForSymbol)
