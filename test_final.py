...
def test_historical_data(symbol):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data['data']:
                latest = data['data'][-1]
                print_info(f"Último preço: ${latest['close']} ({latest['date']})")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data['status'] == 'success':
                ...
                print_info(f"⏱️  Tempo de treinamento: {data['training_time']}s")
...
def main():
    ...
    model_exists = any(m['symbol'] == test_symbol and m['exists'] for m in models_status)
    if not model_exists:
        print_info(f"Modelo para {test_symbol} não existe. Iniciando treinamento...")
