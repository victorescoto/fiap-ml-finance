...
def main():
    ap = argparse.ArgumentParser(
        description="Initialize historical hourly data (30 days) - run once or on-demand")
    ...
    ap.add_argument("--period", default="30d",
                    help="historical period for hourly data (7d, 30d, 60d)")
    args = ap.parse_args()
    ...
    print(
        f"🏗️ HOURLY HISTORICAL INITIALIZATION - Downloading {args.period} data for {len(symbols)} symbols")
    ...
    total_files = sum(len(list(p.rglob("*.parquet")))
                      for p in written_paths if p.exists())
...
def lambda_handler(event, context):
    ...
    symbols = event.get("symbols", os.getenv(
        "SYMBOLS", "AAPL,MSFT,AMZN,GOOGL,META,NVDA,TSLA,DIS"))
