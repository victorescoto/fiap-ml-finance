...
def write_parquet_partitioned(
    ...
    # Renomear Datetime para timestamp se necessário (para dados 5m)
...
def fetch_5m_today(symbol: str):
    df = yf.download(
        tickers=symbol, period="1d", interval="5m", progress=False, threads=False
    )
    if df.empty:
        return pd.DataFrame()
...
def main():
    ...
    base_path = pathlib.Path(args.out) / "prices_5m"
    base_path.mkdir(parents=True, exist_ok=True)

    symbols = [s.strip() for s in args.symbols.split(",") if s.strip()]
    written_paths = []
    for sym in symbols:
        df = fetch_5m_today(sym)
        if df.empty:
            continue
        out = write_parquet_partitioned(df, base_path, "5m", sym)
    ...
    if args.to.startswith("s3://") and not args.dry_run:
        ...
        for p in written_paths:
            for file in p.rglob("*.parquet"):
                key = "/".join(
                    [prefix, "prices_5m", str(file.relative_to((base_path)))]
                )
